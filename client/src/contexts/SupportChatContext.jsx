...
export const SupportChatProvider = ({ children }) => {
    ...
    const registerSocketEvents = useCallback(
        (socket, resolvedName, resolvedUserId) => {
            console.log('[SupportChat] Registering socket events for:', resolvedName);
            ...
            const handleConnectSync = () => {
                setConnected(true);
                const currentConvId = conversationIdRef.current || localStorage.getItem(storageKey);

                if (!currentConvId) {
                    console.log('[SupportChat] Socket connected - No conversation, requesting waiter...');
                    ...
                    console.log('[SupportChat] Socket connected - Rejoining conversation:', currentConvId);
            ...
            socket.on(
                ...
                ({ conversationId, requestStatus: status, message }) => {
                    console.log('[SupportChat] Event conversation:created received:', conversationId);
            ...
            socket.on(
                ...
                ({
                    ...
                    console.log('[SupportChat] Event conversation:joined received:', conversationId);
                    conversationIdRef.current = conversationId;
                    localStorage.setItem(storageKey, conversationId);
                    setMessages(history || []);
                ...
                setMessages((prev) => {
                    ...
                    if (isDuplicate) {
                        return prev.map((existingMsg) => {
                            ...
                            const isMatch = (msg._id && existingMsg._id === msg._id) || 
                                          (existingMsg.text === msg.text && existingMsg._isOptimistic);
                            
                            return isMatch ? { ...msg, _isOptimistic: false } : existingMsg;
                        });
                    }
            ...
            socket.on('reconnect', () => {
                console.log('[SupportChat] Socket reconnected, re-triggering join...');
                const currentConvId = conversationIdRef.current || localStorage.getItem(storageKey);
                if (currentConvId) {
                    socket.emit('customer:join', {
                        conversationId: currentConvId,
    ...
    const sendMessage = useCallback(
        (text) => {
            ...
            if (!socket || !socket.connected) {
                console.warn('[SupportChat] Socket not connected, trying to reconnect...');
                initializeConnection();
                return;
            }

            const name = user?._id ? user.name || 'Khách' : guestName || 'Khách';
            const userId = user?._id || null;

            // Nếu CHƯA có conversationId (lỗi auto-request hoặc do user gõ tin nhắn đầu tiên)
            if (!conversationIdRef.current) {
                console.log('[SupportChat] No conversationId, forcing requestWaiter before sending message');
                socket.emit('customer:requestWaiter', {
                    customerName: name,
                    customerId: userId,
                    tableNumber: null,
                ...
                socket.once('conversation:created', ({ conversationId }) => {
                    console.log('[SupportChat] Conversation created for pending message:', conversationId);
